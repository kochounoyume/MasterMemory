...
`.cs` ファイルは Visual Studio の **Custom Tool（単一ファイルジェネレーター）** である `TextTemplatingFilePreprocessor` によって生成される。生成のタイミングは `dotnet build` 時ではなく、**Visual Studio のデザイン時**（開発者が `.tt` ファイルを保存したとき、または右クリック → "カスタムツールの実行" を選択したとき）である。
...
## `DatabaseBuilderTemplate.cs` が生成される仕組み

`DatabaseBuilderTemplate.cs` は以下の手順で生成される：

### ① `.csproj` での Custom Tool 登録
...
- `<Generator>TextTemplatingFilePreprocessor</Generator>` — Visual Studio に対して、この `.tt` ファイルに **`TextTemplatingFilePreprocessor` という Custom Tool（Single File Generator）** を割り当てることを宣言する。
- `<LastGenOutput>DatabaseBuilderTemplate.cs</LastGenOutput>` — 最後に生成された出力ファイルの名前を記録する（Visual Studio がファイル間の依存関係を追跡するため）。

### ② Visual Studio での生成トリガー

開発者が Visual Studio で `.tt` ファイルを**保存する**か、ソリューションエクスプローラーで右クリック → **「カスタムツールの実行」** を選択すると、Visual Studio が `TextTemplatingFilePreprocessor` を実行する。これは **`dotnet build` では実行されない**（MSBuild とは独立したデザイン時処理）。

### ③ `TextTemplatingFilePreprocessor` による変換

T4 プリプロセッサは `.tt` ファイルを解析し、以下の変換を行って `DatabaseBuilderTemplate.cs` を生成する：

| `.tt` の記述 | 生成される `.cs` の内容 |
|---|---|
| `<#@ import namespace="System.Linq" #>` | ファイル冒頭に `using System.Linq;` を追加 |
| 静的テキスト（例: `namespace ` ） | `TransformText()` 内: `this.Write("namespace ");` |
| `<#= Namespace #>` | `TransformText()` 内: `this.Write(this.ToStringHelper.ToStringWithCulture(Namespace));` |
| `<# foreach(...) { #>` | `TransformText()` 内: C# コードとしてそのまま展開 |

生成クラスの全体像：

```csharp
// DatabaseBuilderTemplate.cs（自動生成・コミット済み）
namespace MasterMemory.GeneratorCore
{
    using System.Linq;    // ← <#@ import namespace="System.Linq" #> から
    using System.Text;
    ...
    public partial class DatabaseBuilderTemplate : DatabaseBuilderTemplateBase
    {
        public virtual string TransformText()
        {
            ...
            this.Write(this.ToStringHelper.ToStringWithCulture(Using));  // <#= Using #>
            // ...
            return this.GenerationEnvironment.ToString();
        }
    }

    // DatabaseBuilderTemplateBase もこのファイルに含まれる（Write(), PushIndent() 等）
...
生成された `DatabaseBuilderTemplate.cs` は **git にコミットされている**。これにより：
- CI/CD や `dotnet build` では `.tt` の処理は行われず、コミット済みの `.cs` がそのままコンパイルされる
- Visual Studio なし（コマンドラインのみ）のビルド環境でも正常にコンパイルできる

開発者が `.tt` を更新した場合は、Visual Studio で Custom Tool を実行して生成された `.cs` を再コミットする必要がある。
...
- `<AutoGen>True</AutoGen>` — 自動生成ファイルであることを示す
- `<DependentUpon>DatabaseBuilderTemplate.tt</DependentUpon>` — VS のソリューションエクスプローラーで `.cs` が `.tt` の子項目として入れ子表示される
- `<DesignTime>True</DesignTime>` — デザイン時生成ファイルであることを示す
...
| MSBuild 要素 | 設定値 | 効果 |
|---|---|---|
| `<None Update="...tt"><Generator>` | `TextTemplatingFilePreprocessor` | VS がこの Custom Tool を `.tt` に割り当てる |
| `<None Update="...tt"><LastGenOutput>` | `DatabaseBuilderTemplate.cs` | 最後の生成出力ファイル名を記録 |
| `<Compile Update="...cs"><AutoGen>` | `True` | 自動生成ファイルとしてマーク |
| `<Compile Update="...cs"><DependentUpon>` | `DatabaseBuilderTemplate.tt` | VS のソリューションエクスプローラーで入れ子表示 |
| `<Compile Update="...cs"><DesignTime>` | `True` | デザイン時生成ファイルとしてマーク |
| `<Service Include="{508349b6-...}">` | T4 サービス GUID | Visual Studio の T4 エンジンとの統合登録 |
